...
public class ShoppingRecordController {
    ...
    @ResponseBody
    public Map<String,Object> addShoppingRecord(int userId,int productId,int counts,int shopId){
        System.out.println("我添加了"+userId+" "+productId+" "+shopId);
        String result = null;
        ...
        if(counts<=product.getCounts()) {
            ...
            System.out.println("在contreller 的 if中 传入之前的 shopId is "+shopId);
            shoppingRecord.setShopId(shopId);
            System.out.println("在contreller 的 if中 shopId is "+shoppingRecord.getShopId());
            Date date = new Date();
...
//        System.out.println("wo在这里i");
       ...
       System.out.println("这是controller");
        for(int i=0;i<shoppingRecordList.size();i++){
           System.out.println(" user is "+shoppingRecordList.get(i).getUserId());
       }
        String shoppingRecords = JSONArray.toJSONString(shoppingRecordList);
        Map<String,Object> resultMap = new HashMap<String,Object>();
        resultMap.put("result",shoppingRecords);
