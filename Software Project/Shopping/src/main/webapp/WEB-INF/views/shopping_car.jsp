...
<script type="text/javascript">
    ...
    function updateShoppingCars() {
        ...
        for(var i=0;i<allShoppingCars.length;i++){
            ...
            counts=allShoppingCars[i].counts;
            //document.write()
            html  += '<tr>'+
                '<td>'+
                '<div class="checkbox">'+
                ...
                '<td>'+product.name+'</td>'+
                '<td>'+product.price+'</td>'+
                '<td>'+'<button type="button" class="btn btn-default" id="subBtn'+allShoppingCars[i].productId+'">-</button>'
                +'<input type="text" class="input-mini" id="text'+allShoppingCars[i].productId+'">'
                +'<button type="button" class="btn btn-default" id="addBtn'+allShoppingCars[i].productId+'">+</button>'
                +'</td >'+'</tr>';

            // shoppingCarTable.innerHTML += html;
            //var productCounts=document.getElementById("productCounts"+i);
            // productCounts.value=allShoppingCars[i].counts;
        ...
        //shoppingCarTable.innerHTML += html;
        for(var i=0;i<allShoppingCars.length;i++){
            document.getElementById("text"+allShoppingCars[i].productId).value=allShoppingCars[i].counts;
            document.getElementById("subBtn"+allShoppingCars[i].productId).onclick=function (ev) { subCount(this.id) }
            document.getElementById("addBtn"+allShoppingCars[i].productId).onclick=function (ev) {addCount(this.id);  }
        }
...
function addCount(id) {
   ...
   // alert(temp);
    ...
    function updateShoppingCar(productCounts,productId) {
        judgeIsLogin();

        //alert(productCounts);
        // var counts = parseInt(productCounts.innerHTML);
        var counts = parseInt(productCounts);
        //alert(counts==0);

        var shoppingCar = {};
        shoppingCar.userId = ${currentUser.id};
       // alert("产品id是："+productId);
    ...
    function addToShoppingRecords(productId,productCounts,bossId) {
        ...
        shoppingRecord.shopId=shopId;
       // alert("shopId is "+shoppingRecord.shopId);
        var buyResult = "";
        $.ajax({
            async : false,
            type : 'POST',
